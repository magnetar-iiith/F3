...
def Nmaxelements(performance_list, models, N):
    ...
    for _ in range(0, N):
        ...
        for j in range(len(performance_list)):
            if performance_list[j]['acc'] / max(performance_list[j]['fnr'], performance_list[j]['fpr']) > obj:
                train_stats = performance_list[j]
                m1 = models[j]
                obj = performance_list[j]['acc'] / max(performance_list[j]['fnr'], performance_list[j]['fpr'])
                idx = j
...
def train(train_loader, agent, out_epoch, inner_runs, wts=None):
    ...
    for epoch in range(inner_runs):
        ...
        for batch_idx, (features, targets, protected) in enumerate(train_loader):
            ...
            ### LOGGING
            if not batch_idx % 50:
                print('Central_Epoch: % 03d/%03d | Agent: % 02d/%02d | Epoch: % 03d/%03d | Batch % 04d/%04d | train_loss: % .4f | EOpp_Loss: % .4f' % (out_epoch+1, config['NUM_EPOCHS'], agent+1, config['NUM_AGENTS'], epoch+1, inner_runs, batch_idx+1,
...
for epoch in range(config['NUM_EPOCHS']):
    ...
    if best_model and config['SAVE_CKPT']:
        ...
        save_everything(1, model_new, optimizer, 1, test_stats, True, config, '{0}-FairAccRatio'.format(K))
